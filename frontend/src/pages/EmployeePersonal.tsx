...
export default function EmployeePersonal({ employeeId: propEmployeeId, onClose }: { employeeId?: number|string, onClose?: ()=>void } = {}){
  ...
  useEffect(()=>{
    ...
    if (!idNum) {
      ...
      api.hrGetEmployee(idNum).then((res:any)=>{
        setEmployee(res)
        const dobVal = res?.dob ? (typeof res.dob === 'string' ? (new Date(res.dob).toISOString().slice(0,10)) : (new Date(res.dob).toISOString().slice(0,10))) : ''
        setForm({
          dob: dobVal,
        ...
        {!loading && (
          <div className="form-card" style={{maxWidth:720}}>
            <form onSubmit={handleSave} style={{display:'grid',gap:12}}>
              ...
              <label>Address</label>
              <input placeholder="Street, City, State, ZIP" value={form.address} onChange={e=>setForm({...form, address: e.target.value})} />
              ...
              <div style={{display:'flex',justifyContent:'flex-end',gap:8}}>
                <button type="button" className="btn-ghost" onClick={()=>{ if (onClose) onClose(); else navigate('/hr/employees') }}>Cancel</button>
                <button type="submit" className="btn-primary">{saving ? 'Saving…' : 'Save'}</button>
              </div>
            </form>
          </div>
        )}
